...
public class CommsTask extends Task {
	
	ItemStack[] icons = new ItemStack[5];

	public CommsTask(Location location) {
		super(location, HeadsItem.RADIO.getHead());
	...
	public void changeItem(Player player, int slot) {
		...
		if (slot == 13 || inventory.getItem(slot) == null) return;
